...
export class AuthController {
    ...
    async register(@Body() userDto: RegisterUserDto, @Res() res: Response) {
        let tokenPair: TokenPairDto = await this._authService.register(userDto);
    ...
    async login(@Body() userDto: LoginUserDto, @Res() res: Response) {
        let tokenPair: TokenPairDto = await this._authService.login(userDto);
    ...
    async refreshToken(@Body() refreshTokenDto: RefreshTokenDto, @Res() res: Response) {
        let tokenPair: TokenPairDto = await this._authService.refreshTokens(refreshTokenDto);
    ...
    @ApiResponse({ status: 400, description: "Some validation error" })
    @ApiResponse({ status: 401, description: "Token is incorrect" })
    @ApiResponse({ status: 500, description: "Unexpected server error" })
    async logout(@Body() refreshTokenDto: RefreshTokenDto, @Res() res: Response) {
        await this._authService.logout(refreshTokenDto);
